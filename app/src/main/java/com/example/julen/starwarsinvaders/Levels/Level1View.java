...
import com.example.julen.starwarsinvaders.Ships.CazaRepublica;
...
public class Level1View extends LevelView {
    ...
    //--CAZA ALIADO--
    CazaRepublica[] cazaAliado = new CazaRepublica[6];
    int numCazaAliado;
    ...
    // disparo aliado
    private DisparoLaser[] disparosAliados = new DisparoLaser[200];
    private int nextBullet2;
    private int maxAliadoBullets2 = 8;
    ...
    public Level1View(Context context, int x, int y, Sensor azelerometroa, SensorManager sm, final String mugimenduAukera, String partidaMota, int score, int lives, StarWarsActivity starWarsActivity) {
    ...
    private void prepareLevel() {
        ...
        //-------CAZA ALIADO-------//
        // Construir la formacion de cazas aliados
        numCazaAliado = 0;
        for (int column = 0; column < 6; column++) {
            for (int row = 0; row < 1; row++) {
                cazaAliado[numCazaAliado] = new CazaRepublica(context, row, column, screenX, screenY);
                numCazaAliado++;
            }
        }
        // Inicializar la formación de disparos de cazas aliados
        for (int i = 0; i < disparosAliados.length; i++) {
            disparosAliados[i] = new DisparoLaser(screenY);
        }
        reproducirCancion();
    ...
    private void update() {
        ...
        for (int i = 0; i < numCazaEnemigo; i++) {
            ...
            if (cazaEnemigo[i].getVisibility()) {
                ...
                //Apuntan a un caza aliado para dispararle
                for (int a = 0; a < numCazaAliado; a++) {
                    if (cazaAliado[a].getVisibility()) {
                        if (cazaEnemigo[i].takeAim(cazaAliado[a].getX(), cazaAliado[a].getLength())) {
                            // Si sí, intentalo y genera una bala
                            if (disparosEnemigos[nextBullet].shoot(cazaEnemigo[i].getX() + cazaEnemigo[i].getLength() / 2, cazaEnemigo[i].getY(), disparoLaser.DOWN)) {
                                // Disparo realizado
                                soundPool.play(disparoLaserID, 1, 1, 0, 0, 1);
                                // Preparete para el siguiente disparo
                                nextBullet++;
        ...
        // que los cazas enemigos disparen a una caza aliado
        for (int i = 0; i < disparosEnemigos.length; i++) {
            if (disparosEnemigos[i].getStatus()) {
                for (int j = 0; j < numCazaAliado; j++) {
                    if (cazaAliado[j].getVisibility()) {
                        if (RectF.intersects(disparosEnemigos[i].getRect(), cazaAliado[j].getRect())) {
                            // A collision has occurred
                            cazaAliado[j].setExplosion();
                            disparosEnemigos[i].setInactive();
                            vibra.vibrate(50);
        ...
        // Actualiza a los cazas ALIADO y la opcion de disparo
        for (int i = 0; i < numCazaAliado; i++) {

            if (cazaAliado[i].getVisibility()) {
                // Mueve el siguiente caza
                cazaAliado[i].update(fps);
                for (int a = 0; a < numCazaEnemigo; a++) {
                    if (cazaEnemigo[a].getVisibility()) {
                        // ¿Quiere hacer un disparo?
                        if (cazaAliado[i].takeAim(cazaEnemigo[a].getX(), cazaEnemigo[a].getLength())) {
                            // Si sí, intentalo y genera una bala
                            if (disparosAliados[nextBullet2].shoot(cazaAliado[i].getX()
                                            + cazaAliado[i].getLength() / 2,
                                    cazaAliado[i].getY(), disparoLaser.UP)) {
                                // Disparo realizado
                                soundPool.play(disparoLaserID, 1, 1, 0, 0, 1);
                                ...
                                // Inicia el ciclo repetitivo otra vez al
                                // primero si ya hemos llegado al último.
                                if (nextBullet2 == maxAliadoBullets2) {
                                    // Esto detiene el disparar otra bala hasta
                                    // que una haya completado su trayecto.
                                    // Por que si bullet 0 todavia está activo,
                                    // shoot regresa a false.
                ...
                // Si ese movimiento causó que golpearan la pantalla,
                // cambia bumped a true.
                if (cazaAliado[i].getX() > screenX - cazaAliado[i].getLength() || cazaAliado[i].getX() < 0) {
                    bumped2 = true;
                }
            }
        }

        // Si un caza aliado toca un borde
        if (bumped2) {
            // Mueve a todos los cazas hacia abajo y cambia la dirección
            for (int i = 0; i < numCazaAliado; i++) {
                cazaAliado[i].dropDownAndReverse();
            }
        }
        // Actualiza a todas las balas de los cazas aliados si están activas
        for (int i = 0; i < disparosAliados.length; i++) {
            if (disparosAliados[i].getStatus()) {
                disparosAliados[i].update(fps);
            }
        }
        // impacto de disparos aliados en la parte superior del borde
        for (int i = 0; i < disparosAliados.length; i++) {
            if (disparosAliados[i].getImpactPointY() < 0) {
                disparosAliados[i].setInactive();
            }
        }

        // impacto de cazas aliados a cazas enemigos
        for (int i = 0; i < disparosAliados.length; i++) {
            if (disparosAliados[i].getStatus()) {
                for (int a = 0; a < numCazaEnemigo; a++) {
                    if (cazaEnemigo[a].getVisibility()) {
                        if (RectF.intersects(disparosAliados[i].getRect(), cazaEnemigo[a].getRect())) {
                            cazaEnemigo[a].setExplosion();
                            soundPool.play(invaderExplodeID, 1, 1, 0, 0, 1);
                            disparosAliados[i].setInactive();
                            cazaDestruida = cazaDestruida + 1;
                        }
    ...
    private void draw() {
        ...
        if (ourHolder.getSurface().isValid()) {
            ...
            if (fondoAtrezo.getVisibility()) {
                canvas.drawBitmap(fondoAtrezo.getBitmap4(), fondoAtrezo.getX(), fondoAtrezo.getY(), paint);
            }
            ...
            //----------------CAZA ALIADO----------------//
            // Dibuja al caza aliado
            for (int i = 0; i < numCazaAliado; i++) {
                if (cazaAliado[i].getVisibility()) {
                    if (cazaAliado[i].getExplosion()) {
                        canvas.drawBitmap(cazaAliado[i].getBitmap(), cazaAliado[i].getX(), cazaAliado[i].getY(), paint);
                    } else {
                        canvas.drawBitmap(cazaAliado[i].getBitmap2(), cazaAliado[i].getX(), cazaAliado[i].getY(), paint);
                        cazaAliado[i].setInvisible();
                    }
                }
            }

            // Dibuja a las balas de los cazas aliados si están activas
            for (int i = 0; i < disparosAliados.length; i++) {
                if (disparosAliados[i].getStatus()) {
                    paint.setColor(Color.parseColor("#27FF00"));
                    canvas.drawRect(disparosAliados[i].getRect(), paint);
                }
            }
